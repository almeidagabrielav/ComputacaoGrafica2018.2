/*
  ...
  Release:     18/11/2018
  Description: Exercício 1 do Slide 4 da disciplna de Computação Gráfica - 2018.2
*/
...
float shoulder = 0.0f, elbow = 0.0f, hand = 0.0f;
...
void display(void)
{
    ...
    glPushMatrix();
	   glRotatef (shoulder, 0.0, 0.0, 1.0); // rotação inicial da base (ombro) do braço
        ...
        glTranslatef (1.0, 0.0, 0.0); // origem posicionada na mao
		glRotatef (hand, 0.0, 0.0, 1.0); // faz rotação em relação a mao
		glTranslatef (1.0, 0.0, 0.0); // posiciona pulso de tamanho 2 na posição correta
		glPushMatrix();
			glScalef (2.0, 0.5, 0.5);
			glutSolidCube (1.0);
		glPopMatrix();
...
int main(int argc, char** argv)
{
   ...
   glutCreateWindow ("Slide 4 - Exercício 1");
   init ();
